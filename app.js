...
async function performDriveSync() {
    ...
    try {
        ...
        const remoteAudioFiles = remoteFolderFiles.filter(isProbablyAudioDriveFile);
        const remoteThumbFiles = await listDriveFilesInFolder(thumbFolderId, "mimeType contains 'image/'");
        ...
        for (const file of remoteAudioFiles) {
            ...
            if (!known) {
                await ingestDriveAudioFile(file, localTrackMap);
            }
        }
        ...
        const finalTracks = Array.from(localTrackMap.values())
            .filter(t => !t.deleted)
            .map(t => {
                const { ...rest } = t;
                return rest;
...
function isProbablyAudioDriveFile(file) {
    if (!file) return false;
    const name = (file.name || '').toLowerCase();
    const mime = (file.mimeType || '').toLowerCase();
    if (mime.startsWith('audio/')) return true;
    const ext = name.includes('.') ? name.split('.').pop() : '';
    return ['mp3', 'm4a', 'aac', 'wav', 'flac', 'ogg', 'oga', 'opus', 'wma', 'alac', 'aif', 'aiff', 'mka', 'mp4', 'm4b'].includes(ext);
}
