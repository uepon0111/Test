...
const App = (() => {
  ...
  async function uploadFiles() {
    ...
    const progress = {
      ...
      detail: document.getElementById('upload-detail-txt'),
      pct: document.getElementById('upload-pct-txt'),
    };
    const setUploadProgress = (pct, detail) => {
      if (progress.wrap) progress.wrap.classList.add('visible');
      if (progress.fill) progress.fill.style.width = Math.max(0, Math.min(100, pct)) + '%';
      if (progress.detail) progress.detail.textContent = detail;
      if (progress.pct) progress.pct.textContent = Math.round(Math.max(0, Math.min(100, pct))) + '%';
    };
    const hideUploadProgress = () => {
      if (progress.wrap) progress.wrap.classList.remove('visible');
      if (progress.fill) progress.fill.style.width = '0%';
      if (progress.detail) progress.detail.textContent = '準備中...';
      if (progress.pct) progress.pct.textContent = '0%';
    };

    setUploadProgress(0, '追加の準備中...');

    const added = [];
    for (let i = 0; i < queue.length; i++) {
      const item = queue[i];
      try {
        setUploadProgress((i / queue.length) * 100, `${i + 1}/${queue.length} 「${item.file.name}」を読み込み中...`);
        const buf = await _readAsBuffer(item.file);

        setUploadProgress((i / queue.length) * 100 + (100 / queue.length) * 0.45, `${i + 1}/${queue.length} 「${item.file.name}」を保存中...`);
        const blobKey = await Storage.saveBlob(buf);

        setUploadProgress((i / queue.length) * 100 + (100 / queue.length) * 0.75, `${i + 1}/${queue.length} 「${item.file.name}」の情報を登録中...`);
        let thumbKey = null;
        if (item.thumbData) {
        ...
        const track = await Storage.addTrack({
          ...
          blobKey,
          thumbKey,
          driveSyncState: Drive.isLoggedIn() ? 'uploading' : null,
        });
        added.push(track);
      } catch (err) {
    ...
    for (const track of added) {
      if (track.driveSyncState === 'uploading') {
