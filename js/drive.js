...
const Drive = (() => {
  ...
  async function syncNow() {
    ...
    try {
      _prog(5, 'Driveフォルダを確認中...');
      await _folders();

      /* Phase A — pull remote data (parallel) */
      _prog(10, 'Google Drive のインデックスを取得中...');
      ...
      const needUpAudio  = afterMerge.filter(t =>  t.blobKey  && !t.driveFileId && t.driveSyncState !== 'uploading');
      const needDlAudio  = afterMerge.filter(t => !t.blobKey  &&  t.driveFileId);
      const needUpThumb  = afterMerge.filter(t =>  t.thumbKey && !t.driveThumbId && t.driveSyncState !== 'uploading');
      ...
      const tick = (detail) => {
        doneIO++;
        _prog(45 + (doneIO / Math.max(totalIO, 1)) * 40, detail || 'ファイルを転送中...');
      };

      _prog(45, 'ファイル転送を開始中...');
      let upA = 0, dlA = 0, upT = 0, dlT = 0;
      await Promise.all([
        _parallel(needUpAudio, async t => { upA++; _prog(45, `音声をアップロード中 (${upA}/${needUpAudio.length})...`); await _uploadAudio(t); tick(`音声をアップロード完了 (${upA}/${needUpAudio.length})`); }, 3),
        _parallel(needDlAudio, async t => { dlA++; _prog(45, `音声をダウンロード中 (${dlA}/${needDlAudio.length})...`); await _downloadAudio(t); tick(`音声をダウンロード完了 (${dlA}/${needDlAudio.length})`); }, 3),
        _parallel(needUpThumb, async t => { upT++; _prog(45, `サムネイルをアップロード中 (${upT}/${needUpThumb.length})...`); await _uploadThumb(t); tick(`サムネイルをアップロード完了 (${upT}/${needUpThumb.length})`); }, 3),
        _parallel(needDlThumb, async t => { dlT++; _prog(45, `サムネイルをダウンロード中 (${dlT}/${needDlThumb.length})...`); await _downloadThumb(t); tick(`サムネイルをダウンロード完了 (${dlT}/${needDlThumb.length})`); }, 2),
      ]);

      /* Phase E — import Drive-direct additions */
      _prog(85, 'Drive から新規ファイルを取り込み中...');
      let imported = 0;
      for (const df of toAdd) {
        imported++;
        _prog(85, `新規ファイルを取り込み中 (${imported}/${toAdd.length})...`);
        await _importDriveFile(df);
        tick(`新規ファイルを取り込み完了 (${imported}/${toAdd.length})`);
      }

      /* Phase F — push updated index */
      _prog(93, '同期インデックスを保存中...');
      await _pushIndex();
  ...
  async function onTrackAdded(track) {
    if (!_token) return;
    try {
      const latest = await Storage.getTrack(track.id);
      if (!latest) return;
      if (latest.driveFileId) {
        if (latest.driveSyncState) await Storage.updateTrack(track.id, { driveSyncState: null });
        return;
      }
      if (latest.driveSyncState !== 'uploading') return;
      await Storage.updateTrack(track.id, { driveSyncState: 'uploading' });
      await _folders();
      // Upload audio and thumb in parallel
      await Promise.all([
        _uploadAudio(latest),
        latest.thumbKey ? _uploadThumb(latest) : Promise.resolve(),
      ]);
      await Storage.updateTrack(track.id, { driveSyncState: null });
      await _pushIndex();
    } catch (e) {
      try { await Storage.updateTrack(track.id, { driveSyncState: null }); } catch {}
