...
const Storage = (() => {
  ...
  async function addTrack(data) {
    ...
    const track = {
      ...
      driveSyncState: data.driveSyncState || null,
  ...
  async function exportSnapshot() {
    ...
    return {
      version:    1,
      exportedAt: Date.now(),
      tracks:     tracks.map(({ blobKey, thumbKey, driveSyncState, ...rest }) => rest),
      playlists, tags, logs,
    };
  }
  async function importSnapshot(snapshot) {
    ...
    for (const t of snapshot.tracks||[]) {
      ...
      if (!ex) await put('tracks', { ...t, blobKey:null, thumbKey:null, driveSyncState:null });
    }
