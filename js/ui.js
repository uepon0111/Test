...
const UI = (() => {
  ...
  async function renderEditGrid() {
    ...
    _editTracks = list;
    grid.style.gridTemplateColumns = `repeat(${_editCols}, 1fr)`;

    grid.innerHTML = list.map(t => {
      const thumbHtml = t._thumbUrl
        ? `<img src="${t._thumbUrl}" style="width:100%;height:100%;object-fit:cover">`
        : '<i class="fa-solid fa-music"></i>';
      const tagDots = (t.tags || []).map(id => {
        const tag = _tags.find(g => g.id === id);
        return tag ? `<span class="tag-dot" style="background:${tag.color}" title="${_esc(tag.name)}"></span>` : '';
      }).join('');
      return `<div class="edit-card" onclick="UI.openEditTrackModal('${t.id}',event)">
        ...
        <div class="edit-card-info">
          <div class="edit-card-title">${_esc(t.title)}</div>
          <div class="edit-card-artist">${_esc(t.artist)}</div>
          <div class="edit-card-tags">${tagDots}</div>
        </div>
      </div>`;
    }).join('');
  }
  ...
  function _renderFileQueue() {
    ...
    if (!qEl) return;
    qEl.innerHTML = _uploadQueue.map((item, i) => `
      <div class="file-queue-item">
        <i class="fa-solid fa-file-audio"></i>
        <span class="file-queue-name">${_esc(item.title)} — ${_esc(item.artist || '不明')}</span>
        <span class="file-queue-size">${_fmtBytes(item.file.size)}</span>
        <button class="file-queue-remove" onclick="UI._removeFromQueue(${i})">
