...
const UI = (() => {
  ...
  async function _moveTrack(id, dir, e) {
    ...
    // 1. Swap in _displayTracks
    [_displayTracks[idx], _displayTracks[newIdx]] = [_displayTracks[newIdx], _displayTracks[idx]];

    // 2. Assign new manualOrder values to all items in the displayed list
    _displayTracks.forEach((t, i) => { t.manualOrder = i + 1; });

    // 3. Sync new order back into master _tracks array (same object references)
    ...
    // 4. Re-render — pass the array directly (no slice, keeps references)
    _playlistVL && _playlistVL.setItems(_displayTracks);
    _updateTrackCount();

    // 5. Persist to IndexedDB (fire-and-forget for responsiveness)
    try {
      await Promise.all(
        _displayTracks.map((t, i) => Storage.updateTrack(t.id, { manualOrder: i + 1 }))
      );
    } catch (err) {
