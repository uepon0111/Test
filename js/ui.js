...
const UI = (() => {
  ...
  function _tagOrderMap() {
    const map = new Map();
    _tags.forEach((tag, idx) => map.set(tag.id, idx));
    return map;
  }
  function _sortedTagIds(ids = []) {
    const order = _tagOrderMap();
    return [...new Set(ids)].sort((a, b) => (order.get(a) ?? 9999) - (order.get(b) ?? 9999));
  }
  function _tagDotHtml(tag, size = 9) {
    return tag ? `<span class="tag-dot" style="background:${tag.color};width:${size}px;height:${size}px" title="${_esc(tag.name)}"></span>` : '';
  }
  ...
  function _renderTrackItem(track, idx) {
    ...
    // Tag dots
    const tagDots = _sortedTagIds(track.tags || []).map(tagId => {
      const tag = _tags.find(t => t.id === tagId);
      return _tagDotHtml(tag);
    }).join('');
  ...
  function _renderAnniversary(tracks) {
    ...
    list.innerHTML = items.map(({ track, years }) => {
      const tagHtml = _sortedTagIds(track.tags || []).map(id => {
        const tag = _tags.find(g => g.id === id);
        return _tagDotHtml(tag, 10);
      }).join('');
  ...
  async function renderEditGrid() {
    ...
    grid.innerHTML = list.map(t => {
      ...
      // Tags: coloured dots only, ordered by tag manager order
      const orderedTags = _sortedTagIds(t.tags || []);
      const tagChips = orderedTags.map(id => {
        const tag = _tags.find(g => g.id === id);
        return _tagDotHtml(tag, 10);
      }).join('');
      const extraTags = orderedTags.length > 6
        ? `<span style="font-size:10px;color:var(--text-muted)">+${orderedTags.length - 6}</span>` : '';
  ...
  async function openEditTrackModal(trackId, e) {
    ...
    _editThumbData   = null;
    _editThumbMime   = null;
    _editCurrentTags = _sortedTagIds(track.tags || []);
  ...
  function _renderEditTagPills() {
    ...
    if (!container) return;
    container.innerHTML = _sortedTagIds(_editCurrentTags).map(id => {
      const tag = _tags.find(g => g.id === id);
      if (!tag) return '';
    ...
    input.addEventListener('keydown', async e => {
      ...
      if (existing) {
        if (!_editCurrentTags.includes(existing.id)) {
          ...
          _editCurrentTags = _sortedTagIds(_editCurrentTags);
          _renderEditTagPills();
        }
        input.value = '';
        return;
  ...
  async function refreshTags() {
    ...
    renderPlaylistTabs();
    applySort();
    if (document.getElementById('log-page')?.classList.contains('active')) renderLogOverview();
    if (document.getElementById('edit-page')?.classList.contains('active')) renderEditGrid();
  }
