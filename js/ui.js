...
const UI = (() => {
  ...
  async function renderEditGrid() {
    ...
    grid.innerHTML = list.map(t => {
      // Thumbnail: img fills the square container (object-fit:cover centres it)
      const thumbHtml = t._thumbUrl
        ? `<img src="${t._thumbUrl}" alt="">`
        : '<i class="fa-solid fa-music"></i>';

      // Release date: show only if set
      ...
      // Tags: coloured dot + name chip (max 3 shown, rest abbreviated)
